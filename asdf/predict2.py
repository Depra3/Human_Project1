...
def run_predict():
    ...
    # geo = gp.read_file('data/seoul_layer.json')
    # with open("data/seoul_layer.geojson", encoding='UTF-8') as response:
    #     geo = json.load(response)
    # m_geo = pd.DataFrame.from_dict(geo)
    # st.write(m_geo)
    

    # m_join = geo.set_index("SIG_KOR_NM").join(m_gu.set_index("SGG_NM"))

    # fig1 = go.Figure(
    #     go.choropleth(
    #         geojson=geo,
    #         locations=m_join.index,
    #         color="RENT_GTN",
